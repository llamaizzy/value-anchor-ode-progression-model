...
simulate_true_rate <- function(YL = 0.30, YU = 1.70, K = 8, nGrid = 201,
  ...
  Bgrid <- splines::bs(ygrid, knots = interiorKnots, Boundary.knots = c(YL, YU), degree = 3, intercept = TRUE)
...
simulate_design <- function(N = 200,
                            ...
                            sigma_eps_true = 0.05,
                            x0_range = c(0.6, 1.3),   # anchor SUVR value range
                            baseline_age_range = c(40, 90),      # baseline age range (yrs)
                            visits_range = 2:6,
